...
(async () => {
  ...
  if (mode === 'stills') {
    ...
    for (const t of arg.split(',').map(Number)) fs.writeFileSync(path.join(dir, `t${t.toFixed(2)}.png`), await grab(Math.round(t * 24)));
  } else {
    const total = await page.evaluate(() => window.FRAMES);
    const ff = spawn(FFMPEG, ['-y', '-loglevel', 'error', '-f', 'image2pipe', '-framerate', '24', '-c:v', 'png', '-i', '-',
    ...
    for (let f = 0; f < total; f++) {
      ...
      if (f % 48 === 0) console.log(`frame ${f}/${total}  ${((Date.now() - t0) / 1000).toFixed(0)}s`);
    }
    ff.stdin.end();
